...
let interval = setInterval(() => {
  ...
  if(letter.length > 0){
    ...
    // Sync render during streaming for real-time feedback
    state.content = common.formatMarkdown(originContent);
  }
  if(utils.isEqual(letter.length, 0) && isEndStream){
    clearInterval(interval);
    // After streaming completes, switch to async worker render for better performance
    renderMarkdownAsync();
  }
}, 50)

async function renderMarkdownAsync() {
  try {
    state.renderedContent = await formatMarkdownAsync(originContent);
  } catch {
    state.renderedContent = common.formatMarkdown(originContent);
  }
}
// Initial async render on mount
renderMarkdownAsync();

function getContent(content){
  // Return async rendered content; fallback to sync if not ready yet
