...
const innerEmit = defineEmits(['oncancel', 'onnav']);
...
function onNavChat(){
  ...
  innerEmit('onnav', { conversationType, conversationId });
  state.content = '';
}
...
<template>
  <DialogRoot :open="props.isShow" @update:open="(v) => !v && emit('oncancel')">
    <DialogContent root-class="modal tyn-modal tyn-search-modal" :class="[props.isShow ? 'fade show' : '']">
                      ...
                      <li class="jg-search-child">
                        <li class="tyn-aside-item js-toggle-main" v-for="(item, index) in state.conversations"
                          ...
                          <div class="tyn-media-group">
                            <div class="tyn-media jg-size-rg">
