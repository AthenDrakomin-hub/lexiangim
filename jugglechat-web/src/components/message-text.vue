...
watch(() => props.message, (msg) => {
  ...
  renderMarkdownAsync(msg);
});

// Render markdown asynchronously via Web Worker when component mounts or message changes
async function renderMarkdownAsync(msg) {
  ...
  try {
    state.renderedContent = await formatMarkdownAsync(raw);
  } catch {
    state.renderedContent = common.formatMarkdown(raw);
  }
}
renderMarkdownAsync(props.message);
...
function getContent(content){
  // Keep sync fallback for translation and inline editing; primary content uses async rendering
  ...
  <div class="tyn-reply-group" >
    ...
    <div class="tyn-reply-bubble" :messageid="props.message.messageId" :messageId="props.message.tid">
      ...
      <div class="tyn-reply-text wr" v-else v-longpress="onClickRight" @click.right.prevent="onClickRight" @click.prevent="onShowEmojiReaction(true)">
        ...
        <span v-html="state.renderedContent"></span>
