...
let state = reactive({
  ...
  messageHeights: {}, // dynamic height map for DynamicScrollerItem
  mentionMembers: [
...
function onMessageResize(message, size) {
  state.messageHeights[message.messageId] = size;
}
  ...
  <div class="tyn-main tyn-chat-content aside-collapsed"
    ...
    <div class="tyn-chat-head">
      ...
      <div class="tyn-media-group">
        <div class="tyn-media jg-size-md tyn-conver-avatar" :style="{ 'background-image': 'url(' + props.conversation.conversationPortrait + ')' }">
        </div>
...
<div class="tyn-chat-body js-scroll-to-end" ref="messages" :class="{'tyn-h5-chat-body': !utils.isUniapp()}">
      ...
      <DynamicScroller
        ...
        class="tyn-reply"
      >
        <template v-slot="{ item: message, active }">
          <DynamicScrollerItem
            :item="message"
            :active="active"
            :size-dependencies="[message.content, message.translation, message.reactions]"
            @resize="onMessageResize(message, $event)"
          >
