...
func DeleteUser(appkey, userId string) errs.AdminErrorCode {
	if appkey == "" || userId == "" {
		return errs.AdminErrorCode_ParamError
	}
	storage := dbs.UserDao{}
	...
	err = storage.DeleteUser(appkey, userId)
	if err != nil {
		logs.NewLogEntity().Error(err.Error())
		return errs.AdminErrorCode_Default
	}
...
func ResetUserPassword(appkey, userId, newPassword string) errs.AdminErrorCode {
	if appkey == "" || userId == "" || newPassword == "" {
		return errs.AdminErrorCode_ParamError
	}
	...
	hashedPassword := tools.SHA1(newPassword)
	err = storage.UpdateUserPassword(appkey, userId, hashedPassword)
	if err != nil {
		logs.NewLogEntity().Error(err.Error())
		return errs.AdminErrorCode_Default
	}
